...
#         ''')
    ...
    def get_res_users(self, cr, uid, user_id , context=None, **kwargs):
        cr.execute('''
            ...
            allow_daily_sale_report,allow_pre_sale,allow_direct_sale,allow_assets,allow_customer_location_update,allow_stock_check,allow_rental,allow_feedback,allow_customer_create
            from res_users 
            where id = %s
            ''', (user_id,user_id,))
        datas = cr.fetchall() 
        ...
        try:
            if list_val:
                for So_id in list_val:
                    ...
                    so.invoiced,so.branch_id,so.delivery_remark ,team.name,so.payment_term,so.due_date,so.rebate_later,rp.township ,rt.name as township_name
                    from sale_order so, crm_case_section team ,res_township rt ,res_partner rp                                
                    where so.id= %s and rp.id =so.partner_id and rt.id=rp.township
                    and so.state!= 'cancel'
